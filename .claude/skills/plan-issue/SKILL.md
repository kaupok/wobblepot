...
- A size, height, padding, or radius default on a primitive under `src/components/ui/*.tsx` — however it is expressed. Only `button.tsx` uses `cva`; `input.tsx` carries its height in a plain class string and `select.tsx` in a `data-[size=…]` selector, so "a CVA variant" would skip two thirds of the worked example below.
...
grep -n 'extendTailwindMerge\|twMerge' src/lib/utils.ts
```
