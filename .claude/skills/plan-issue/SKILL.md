...
# 6. If the new value uses a custom `--spacing-*` token, confirm it is registered
#    with tailwind-merge — otherwise every override grep 3 found silently keeps
#    both classes and the cascade picks the winner.
grep -rn 'extendTailwindMerge\|twMerge' src/lib/utils.ts src/lib/utils.test.ts
```
...
- **Grep 6 is the one that has no callsite at all.** `cn()` runs every `className` override through tailwind-merge, which only knows Tailwind's built-in scale. Register a custom token and it works; forget to, and `cn('h-8', 'h-touch')` returns `"h-8 h-touch"` — both classes survive, the cascade picks a winner, and every override grep 3 surfaced is quietly voided with no failing test. PR #704 had to add `extendTailwindMerge({ extend: { theme: { spacing: ['touch'] } } })` to `src/lib/utils.ts` for exactly this reason. That file and its colocated test are the only `src` files that PR touched which no other grep here can reach — greps 1–4 are `--include='*.tsx'` and grep 5 is scoped to `src/components/ui`. (HON-626 tracks turning this into a standing guard rail.)
