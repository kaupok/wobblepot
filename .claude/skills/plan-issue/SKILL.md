...
**How:** for each changed primitive, take the **old** literal class value it is moving away from — the one callsites would have copied — and find every hardcoded copy. The greps below are written for a **height/size** change (`h-9` / `size-9` → substitute your own). For a padding or radius change, substitute the property in greps 1–3 as well (`p-`/`px-`, `rounded-`); note that greps 1 and 3 then need their value list widened, and that `rounded-md` doubles as the classification tell below, so the output is far noisier. For a `@theme` token change the token *name* is the literal, so scan for it directly (`grep -rn 'spacing-touch' src`) — that finds the consumers the tag-anchored greps cannot, including `tokens.stories.tsx:224`, which asserts the token's px value against a hardcoded `TOUCH_TARGET_PX = 44` and fails `pnpm test-storybook:ci` if the two drift. For a shared layout wrapper, only grep 2 applies: grep the container literal (`max-w-3xl`, `px-4`) and diff each route's `page.tsx` against its `loading.tsx`, which are written to match byte for byte — `admin/signup-codes/page.tsx:51` and `loading.tsx:9` both read `mx-auto flex max-w-3xl flex-col gap-6 p-6`. **The `none` line is only honest for the property you actually scanned; never write it for a property the greps did not cover.**

```bash
# 1. Skeletons that mirror control geometry — matched by shape, not by location.
grep -rn '<Skeleton' src --include='*.tsx' \
  | grep -E '\b(min-h|h|size)-(5|6|7|8|9|10|11|12|14|touch)\b' \
...
[From step 7b. Group by Mirror / Override / Deliberate with a `file:line` each and a one-line reason. If the scan ran and found nothing, name the property you scanned — `none — no callsite hardcodes the changed height` — rather than the unqualified "geometry", so a change that moved height and padding but was only scanned for height cannot read as a clean full scan. Omit this section entirely only if 7b was skipped (step 6 surfaced no shared-primitive geometry change).]
