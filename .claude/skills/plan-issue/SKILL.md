...
**How:** for each changed primitive, take the **old** literal class value it is moving away from — the one callsites would have copied — and find every hardcoded copy. The greps below are written for a **height/size** change (`h-9` / `size-9` → substitute your own). For a padding or radius change, substitute the property in greps 1–3 as well (`p-`/`px-`, `rounded-`); note that greps 1 and 3 then need their value list widened, and that `rounded-md` doubles as the classification tell below, so the output is far noisier. For a `@theme` token change, scan for the **generated utility**, not the CSS variable: `--spacing-touch` is written `h-touch` / `size-touch` / `min-h-touch` at every callsite, so `grep -rn 'spacing-touch' src` returns 5 lines (the definition and some prose) while `grep -rnE '\b(min-h|h|size)-touch\b' src` returns the 44 real consumers across 23 files. Add one grep for the variable name itself, which catches the readers no utility grep can — `tokens.stories.tsx:224` reads `--spacing-touch` through `getComputedStyle` and asserts it against a hardcoded `TOUCH_TARGET_PX = 44`, failing `pnpm test-storybook:ci` when the two drift. For a shared layout wrapper, only grep 2 applies: grep the container literal (`max-w-3xl`, `px-4`) and diff each route's `page.tsx` against its `loading.tsx`, which are written to match byte for byte — `admin/signup-codes/page.tsx:51` and `loading.tsx:9` both read `mx-auto flex max-w-3xl flex-col gap-6 p-6`. **The `none` line is only honest for the property you actually scanned; never write it for a property the greps did not cover.**
