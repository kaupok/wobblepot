...
- **Storybook with an accessibility gate.** Over eighty colocated story files run through axe in CI, and modal stories carry play functions that assert focus trapping, Escape handling, and tab order. Browse the published build at [kaupok.github.io/wobblepot](https://kaupok.github.io/wobblepot/), rebuilt from `main` on every merge. See [`.storybook/README.md`](.storybook/README.md).
