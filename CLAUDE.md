...
**Migration immutability: a migration file is frozen once it is on `main`.** By then it has been applied somewhere durable — `deploy-db-migrations-staging.yml` runs `prisma migrate deploy` on every merge, and production applies the same files — and Prisma stores a checksum of each `migration.sql` at apply time, so editing one only guarantees that `prisma migrate dev` will demand a full database reset (HON-558). Never edit, delete, or renumber an applied migration; add a new one that fixes forward. CI enforces this on every PR, and checks every push to `main` against the previous tip, so an edit merged past a red or cancelled PR check turns that push's `main` run red (HON-649 — detection only until HON-584 makes checks required). Only that run: the next unrelated merge goes green with the drift still in place, so act when it fires. If that happens, **revert** the offending commit rather than fixing forward: staging and production still hold the checksum of the original SQL, so only restoring it clears the drift; the revert itself trips the check once more (on its PR and its `main` push), which is expected. Run it locally with `git fetch origin main && bash scripts/check-migrations-immutable.sh origin/main` — fetch first, because a stale `origin/main` omits migrations that landed since, and editing one of those then reads as an addition and passes.
