...
export class EventStreamer {
  ...
  private async getEdgeWorkersForWorkspace(workspaceId: string): Promise<string[]> {
    ...
    for (const token of edgeTokens) {
      const connectionData = await this.env.EDGE_TOKENS.get(`edge:connection:${token}`)
      if (connectionData) {
   ...
   */
  async trackEdgeConnection(linearToken: string, workspaceIds: string[]): Promise<void> {
