...
import { EdgeWorkerRegistry, type StoredEdgeWorker } from './EdgeWorkerRegistry.js'
...
export class WebhookSender {
   ...
   */
  async sendWebhookToWorkspace(event: EdgeEvent, workspaceId: string): Promise<number> {
    const edgeWorkers = await this.registry.getEdgeWorkersForWorkspace(workspaceId)
   ...
   */
  private async makeWebhookRequest(worker: StoredEdgeWorker, event: EdgeEvent): Promise<void> {
    const body = JSON.stringify(event)
    const timestamp = Date.now().toString()
    const signature = await this.generateWebhookSignature(body, timestamp, worker.webhookSecret)
