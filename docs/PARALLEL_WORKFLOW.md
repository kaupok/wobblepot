...
- Fetch Todo issues via Linear GraphQL (curl + jq), capped at `LINEAR_TODO_PAGE_SIZE` (50) per poll. The query asks for one row past the cap, so a deeper queue is detectable, and logs `WARN Todo queue exceeds the 50-issue query cap` when it comes back. There is no pagination and does not need to be — a poll claims at most one issue — but before HON-580 the truncation was silent, so the orchestrator's view of "what is available" quietly stopped matching Linear's
