...
| Review harness  | Storybook (`pnpm storybook`), mobile viewport by default, plus `@shadcn/lint` in `pnpm lint`                                            | Every component change ships with a story. Axe runs on every story in CI. `Scenarios/*` stories compose whole screens, and each one runs `assertDesignRules` from `src/stories/design-rules.ts` over its DOM, so the mechanical rules below fail CI rather than a review (HON-610). `@shadcn/lint` covers the static half from the other side — six rules as errors on every `src` file, story or not, whether or not a scenario renders it (HON-673). The sixth, `no-restyle`, reports a `className` that changes what a primitive owns — its padding, radius, colour, type, or a control's height — and points at the variant or size to use instead; the per-component contracts in `eslint.config.mjs` say what a page may still change, each with its reason (HON-674). The two overlap on raw colours and complement each other everywhere else: the linter sees files no story reaches, the DOM helper sees classes assembled at runtime. Keep both |
...
- **The image on a surface.** Rendered through `next/image` with `object-fit: cover` and `mix-blend-mode: multiply`, so the white surface of the illustration takes the tint and the plate sits on the card rather than in a white box. A `mask-image` linear fade blends its edge into the surface:
  - **Cards:** the image sits on the right at about 62% of the card width (45% on a card narrower than 448px, measured with a container query so a narrow grid column counts as narrow), and the fade runs left to right, reaching full opacity at 70% of the image width. The text sits on the plain tint to its left, and the title is capped in width so it wraps before it reaches the image. A card with actions at the right end of its title row (the planner card's Note and menu) reserves that column: the image ends before the actions, so nothing in the title row, read or tapped, sits on the opaque illustration (`trailingActions` on `MealImageCard`, HON-749). A card with rows below its title (the planner card's note, status control and rating) confines the image to the title row's band: it runs from the card's top edge to where the second row starts, with a short fade at its bottom, so every row below the title sits on the plain tint at the full card width (`titleBand` on `MealImageCard`, HON-755). On a wide card the band shows a slice of the plate rather than the whole dish; that is the price of keeping the status control and the rating prompt full width. A card taller than wide (the add-meal dialog's alternatives) uses the bottom layout instead: the image is a full-width 3:2 block below the content and above the actions, fading upward into the tint over its top 40%, and the title keeps the full row (`layout="bottom"`, HON-750).
  - **Modal hero:** 2:1 (the 3:2 image cropped with `object-fit: cover`), full-bleed: a negative margin equal to the dialog padding runs it edge to edge across the dialog, with no radius of its own, so the tint reads as the dialog's header band rather than an inset card (HON-752). First element of `MealDetail` (below the title and the note, above the description). The fade runs bottom-up into the content below, and a short top fade (the top 15%) keeps the band from starting at a hard line under the note.
...
- **Absence renders the neutral card.** A meal without an image renders the ordinary neutral card and no hero: no tint, no image element, no placeholder, icon, skeleton, or shimmer. A meal with an image but no `imageHue` (food with no chroma to take a hue from) renders the image **on a neutral surface** — the tint's lightness at zero chroma (`data-meal-surface="neutral"`), not the plain `--card`, which in the dark theme would multiply the image to near-black — on the card and the hero alike: a picture that was generated and paid for is never hidden by its colour. A placeholder tells the user something is missing when nothing is. The one exception is the hero while `imageStatus` is `generating`: a plain `bg-muted` box with the hero's full-bleed 2:1 geometry holds the space so the content does not jump when the image lands. The box has no animation and no icon. Cards show nothing extra while generating.
