...
| Page shell      | `src/app/layout.tsx`, `header.tsx`, `bottom-tab-bar.tsx`                           | Fixed header, bottom tab bar on mobile, content column capped at 1152px (`HON-376`)                                                                             |
...
| Section   | Day names, form sections ("Ingredients")       | `<Heading variant="section" as="h2">` | `text-base font-semibold`       |
| Body      | Meal names, ingredients, descriptions, links   | `<Body variant="small">`              | `text-sm font-medium`           |
| Secondary | Helper text, subtitles, summaries              | `<Body variant="muted">`              | `text-sm text-muted-foreground` |
...
- `Heading` `h1`, `h2`, `h3` are for the marketing landing page, legal pages, and error pages. Inside the app, page titles are `h4`. Five in-app components still use `h2` (`ShoppingEmptyState`, `MemberList`, `FirstTimeSetup`, `GeneratingOverlay`, `HouseholdSettingsForm`); decided 2026-09-03 to migrate them, not to add an exception.
...
- Interactive list rows and tab items are at least 44px tall on mobile (`min-h-[44px]` today; **Proposed:** a named `min-h-touch` utility, HON-609).
...
- HON-608: add `success`, `warning`, `info` tokens and migrate the 25 files using raw palette classes (color).
