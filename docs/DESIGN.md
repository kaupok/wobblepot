...
- `Checkbox` and `RadioGroupItem` are 20px (`size-5`) by default, sized for the 16px body text they sit beside (HON-687); no callsite overrides the checkbox size except `IngredientList`'s compact mode (`size-4`). In `ShoppingItem` the `<label>` wraps the whole 54px row, so the real target is the row; in `CustomShoppingItem` the label covers only the 28px text block, so the checkbox is closer to a bare 20px target.
