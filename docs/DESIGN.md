...
| Review harness  | Storybook (`pnpm storybook`), mobile viewport by default                                                                                | Every component change ships with a story. Axe runs on every story in CI. `Scenarios/*` stories compose whole screens, and each one runs `assertDesignRules` from `src/stories/design-rules.ts` over its DOM, so the mechanical rules below fail CI rather than a review (HON-610) |
...
- Controls still below the floor are a known gap, not an exception. HON-612 raised the size variants (measured at 390px: `Button` default 44, `lg` 48, `icon` 44, `icon-lg` 48; `sm` 32 and `icon-sm` 32 by design), which by construction reaches only elements that take their height from the variant. Everything still smaller is one of three things — a control pinned below the floor through a _variant_ rather than a `className`, which is deliberate and invisible to a future variant raise (`PantryItem` and `PantrySection` set their star and trash actions to `icon-sm` for the 32px card-action rhythm), a raw `<button>` that never goes through `Button` (`MealCard`'s meal name 15px, `NoteEditor`'s "Add note" 20px, `CustomShoppingItem`'s unlink and delete 22px, `MealRatingInline`'s thumbs and `RatingBadge` 24px), or a `Button` whose `className` overrides the size variant (`MealCard`'s Note / Swap / Clear 20px via `h-5` and "Add meal" 28px via `h-7`, `MealRatingPrompt`'s thumbs 28px via `h-7` and dismiss 24px via `h-6`). Those two groups are most of this list and are still open. Full measurements are in the HON-609 and HON-612 PRs.
...
- **Durations are a set, not a range:** 150ms for control feedback (press, hover, colour), 200ms for menus, dialogs, and every exit, 300ms for a sheet opening. Nothing inside the app runs longer. An exit is never slower than its enter. If a motion needs another value, the motion is wrong, not the set.
...
- **Transitions for state, keyframes for entrances.** A state change that can be reversed midway (open/close, toggle, press) uses a CSS transition, which retargets. `animate-in` keyframes restart from zero, so they are for one-shot entrances only.
...
- `transition-all`, `ease-in`, or any animation longer than 300ms inside the app
