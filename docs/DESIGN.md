...
- Elevation: only overlays cast a shadow. `shadow-md` on anchored overlays (select, dropdown sub-menu, autocomplete lists), `shadow-lg` on modal overlays (dialog, alert dialog, sheet) and the top-level dropdown. Nothing in the page — controls, cards, badges, rows — has one; the border is the edge. HON-689 removed shadcn's default `shadow-xs` / `shadow-sm` from controls and `Card`, so a `shadcn add` can bring them back — strip them. The `no-content-shadow` scenario rule fails on any visible shadow outside an element with `role="dialog" | "alertdialog" | "menu" | "listbox"` or a `data-slot` ending in `-content` (a hand-rolled popover gets `data-slot="autocomplete-content"`); focus rings are spread-only and do not count. The grep below must return only overlays — `dialog`, `alert-dialog`, `sheet`, `dropdown-menu`, `select` (content only), `InlineAddItem`, `UnmatchedIngredientRow`, `IngredientSearch`:
