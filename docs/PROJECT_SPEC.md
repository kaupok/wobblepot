...
- [ ] Legal: Privacy Policy + Terms published and versioned consent captured at sign-up (HON-457); DPAs in place with Anthropic, Resend, Vercel, PostHog (HON-459, [`compliance/README.md`](../compliance/README.md)). **Open:** Neon's DPA is auto-incorporated via its ToS only — countersigned copy and billing-entity migration are HON-553; the DPIA's allergen-entry affirmation is not built ([Risk area 2](../compliance/dpia.md))
...
- [ ] Email: sending domain, FROM addresses and setup documented in [`docs/EMAIL_SETUP.md`](./EMAIL_SETUP.md) (HON-465). **Open:** SPF/DKIM/DMARC aligned (unverified from repo: the DNS records and Resend's "Verified" status live in Cloudflare and Resend); password-reset reliably lands in inbox (unverified from repo: HON-465's Mail-Tester, inbox-placement and production reset checks were never recorded)
- [ ] CI safety net: E2E re-enabled in CI (HON-455, HON-518); sign-up (`auth.spec.ts`) and shopping→pantry (HON-479) covered in tier 1. **Open:** generation is covered only by `@ai` specs, which no CI tier runs ([`tests/e2e/README.md`](../tests/e2e/README.md)); the household invite flow has no spec since HON-518 deleted the stale `invite.spec.ts`
- [ ] Launch hygiene: `/api/health` (HON-454); `not-found.tsx`, `robots.ts`, `sitemap.ts` and OG/Twitter metadata (HON-456; legal routes added to the sitemap in HON-559). **Open:** uptime monitor (HON-484); the `/og-image.png` the OG metadata references does not exist (HON-483)
